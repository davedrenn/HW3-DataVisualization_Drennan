...
This communication starts by providing a graph title designed as a sentence. The need for a chart legend is eliminated by using the same color for a keyword in the title to connect it with the color scheme of the graph. This use of color is more necessary in the first two charts given the lack of a legend, but is also designed as a pre-attentive attribute to focus the reader on the important data. 

Connectivity is used with captions in the second chart to provide contextual events that can help explain some of the spikes and dips of positive cases over the course of the pandemic. Each caption's distance from the x-axis is also supposed to mimic the rise, fall, and slight uptick in cases to visually reinforce the argument that this analysis is trying to communicate.

All three graphs have minimal use of borders, axes, and other non-data features that make charts busier. The use of color is minimized to highlight key details.
With the use of these different gestalt and pre-attentive techniques, I believe that these charts achieve the goal of communicating the potential increased risk of catching COVID this winter.
